...
static std::filesystem::path locate_hook_library() {
    ...
    candidates.emplace_back(std::filesystem::current_path() / "build" / "lib" / "libhook.dylib");
    candidates.emplace_back(std::filesystem::current_path() / "build" / "libhook.dylib");
    candidates.emplace_back(std::filesystem::current_path() / "libhook.dylib");
...
static int run_macos_dynamic_analysis(const std::filesystem::path& directory,
    ...
    fs::path hook = locate_hook_library();
    if (hook.empty() || !fs::exists(hook)) {
        std::cerr << "[dynamic_analysis] unable to locate libhook.dylib" << '\n';
        return 1;
    }
    ...
    std::cout << "[dynamic_analysis] host: macOS" << '\n';
    std::cout << "[dynamic_analysis] preload: " << hook << '\n';
    std::cout << "[dynamic_analysis] target:  " << target << '\n';
    std::cout << "[dynamic_analysis] log:     " << log_file << '\n';
