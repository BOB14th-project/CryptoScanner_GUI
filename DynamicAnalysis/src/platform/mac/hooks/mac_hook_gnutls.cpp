// src/platform/mac/hooks/mac_hook_gnutls.cpp
// Capture symmetric key usage from selected GnuTLS cipher APIs on macOS.
...
#error "mac_hook_gnutls.cpp requires GnuTLS headers"
#endif
...
static void* resolve_gnutls_symbol(const char* name) {
    ...
    if (const char* verbose = std::getenv("HOOK_VERBOSE"); verbose && *verbose == '1') {
        std::fprintf(stderr, "[hook_macos] gnutls symbol '%s' unresolved\n", name);
    }
    return nullptr;
}
...
#define RESOLVE_SYM(var, name_literal)                                               \
    do {                                                                             \
        if (!(var)) {                                                                \
            (var) = reinterpret_cast<decltype(var)>(resolve_gnutls_symbol(name_literal)); \
        }                                                                            \
    } while (0)
...
using fn_gnutls_cipher_get_name = const char* (*)(gnutls_cipher_algorithm_t);
static fn_gnutls_cipher_get_name real_gnutls_cipher_get_name = nullptr;
...
static std::string cipher_name_from_alg(gnutls_cipher_algorithm_t alg) {
    RESOLVE_SYM(real_gnutls_cipher_get_name, "gnutls_cipher_get_name");
    const char* name = real_gnutls_cipher_get_name ? real_gnutls_cipher_get_name(alg) : nullptr;
...
DYLD_INTERPOSE(gnutls_cipher_init, gnutls_cipher_init)
...
DYLD_INTERPOSE(gnutls_cipher_deinit, gnutls_cipher_deinit)
...
DYLD_INTERPOSE(gnutls_cipher_set_iv, gnutls_cipher_set_iv)
...
DYLD_INTERPOSE(gnutls_cipher_encrypt, gnutls_cipher_encrypt)
...
DYLD_INTERPOSE(gnutls_cipher_encrypt2, gnutls_cipher_encrypt2)
...
DYLD_INTERPOSE(gnutls_cipher_decrypt, gnutls_cipher_decrypt)
...
DYLD_INTERPOSE(gnutls_cipher_decrypt2, gnutls_cipher_decrypt2)
...
DYLD_INTERPOSE(gnutls_cipher_add_auth, gnutls_cipher_add_auth)
...
DYLD_INTERPOSE(gnutls_cipher_tag, gnutls_cipher_tag)
