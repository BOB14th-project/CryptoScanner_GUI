                                   ...
                                   const OSSL_PARAM* params)
{
  ...
  if (std::getenv("HOOK_VERBOSE")) {
    const EVP_CIPHER* c = type ? type : cipher_from_ctx(ctx);
    const char* cname = cipher_name(c);
    fprintf(stderr, "[hook_macos] ex2 logged cipher=%s keyptr=%p ivptr=%p\n",
            cname ? cname : "(null)", (const void*)key, (const void*)iv);
  }
  return real_EVP_EncryptInit_ex2(ctx, type, key, iv, params);
}
DYLD_INTERPOSE(EVP_EncryptInit_ex2, EVP_EncryptInit_ex2)
...
DYLD_INTERPOSE(EVP_DecryptInit_ex2, EVP_DecryptInit_ex2)
...
DYLD_INTERPOSE(EVP_CipherInit_ex2, EVP_CipherInit_ex2)
...
DYLD_INTERPOSE(EVP_CIPHER_CTX_set_params, EVP_CIPHER_CTX_set_params)
...
DYLD_INTERPOSE(EVP_CIPHER_CTX_get_params, EVP_CIPHER_CTX_get_params)
