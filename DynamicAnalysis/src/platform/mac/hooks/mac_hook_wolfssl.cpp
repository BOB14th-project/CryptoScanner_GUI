...
static void* resolve_wolfssl_symbol(const char* name) {
    ...
    static const char* kCandidates[] = {
        "libwolfssl.dylib",
        "libwolfssl.25.dylib",
        "/opt/homebrew/lib/libwolfssl.dylib",
        "/usr/local/lib/libwolfssl.dylib",
        nullptr
    };
    ...
    if (const char* verbose = std::getenv("HOOK_VERBOSE"); verbose && *verbose == '1') {
        std::fprintf(stderr, "[hook_macos] wolfssl symbol '%s' unresolved\n", name);
    }
    return nullptr;
}
...
#define RESOLVE_SYM(var, name_literal)                                               \
    do {                                                                             \
        if (!(var)) {                                                                \
            (var) = reinterpret_cast<decltype(var)>(resolve_wolfssl_symbol(name_literal)); \
        }                                                                            \
    } while (0)
...
DYLD_INTERPOSE(wc_AesGcmSetKey, wc_AesGcmSetKey)
...
DYLD_INTERPOSE(wc_AesGcmSetKey_ex, wc_AesGcmSetKey_ex)
...
DYLD_INTERPOSE(wc_AesGcmEncrypt, wc_AesGcmEncrypt)
...
DYLD_INTERPOSE(wc_AesGcmDecrypt, wc_AesGcmDecrypt)
...
DYLD_INTERPOSE(wc_AesSetKey, wc_AesSetKey)
...
DYLD_INTERPOSE(wc_AesSetIV, wc_AesSetIV)
...
DYLD_INTERPOSE(wc_AesCbcEncrypt, wc_AesCbcEncrypt)
...
DYLD_INTERPOSE(wc_AesCbcDecrypt, wc_AesCbcDecrypt)
...
DYLD_INTERPOSE(wc_AesFree, wc_AesFree)
...
DYLD_INTERPOSE(wc_HmacSetKey, wc_HmacSetKey)
...
DYLD_INTERPOSE(wc_HmacFinal, wc_HmacFinal)
...
DYLD_INTERPOSE(wc_HmacFree, wc_HmacFree)
